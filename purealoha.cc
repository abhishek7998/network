...
void UanMacAloha::DoDispose ()
{
	Clear ();
	UanMac::DoDispose ();
}
 
TypeId UanMacAloha::GetTypeId (void)
{
...
bool UanMacAloha::Enqueue (Ptr<Packet> packet, uint16_t protocolNumber, const Address &dest)
{
...
void UanMacAloha::SetForwardUpCb (Callback<void, Ptr<Packet>, uint16_t, const Mac8Address&> cb)
{
	m_forUpCb = cb;
}
void UanMacAloha::AttachPhy (Ptr<UanPhy> phy)
{
	m_phy = phy;
...
void UanMacAloha::RxPacketGood (Ptr<Packet> pkt, double sinr, UanTxMode txMode)
{
	NS_UNUSED (sinr);
	UanHeaderCommon header;
...
void UanMacAloha::RxPacketError (Ptr<Packet> pkt, double sinr)
{
	NS_LOG_DEBUG ("" << Simulator::Now () << " MAC " << Mac8Address::ConvertFrom (GetAddress ()) << " Received 			packet in error with sinr " << sinr);
}
 
int64_t UanMacAloha::AssignStreams (int64_t stream)
{
